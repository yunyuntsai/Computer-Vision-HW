...
def ComputeError(H, pt1, pt2, match):
    ...
    pt1_data = pt1[match[:,0],:]
    pt1_data = np.hstack([pt1_data,np.ones([pt1_data.shape[0],1])])
    pt2_data = pt2[match[:,1],:]
    pt2_data = np.hstack([pt2_data,np.ones([pt2_data.shape[0],1])])
    dist = (pt2_data - np.matmul(pt1_data,H.T))
    dists = np.linalg.norm(dist,ord=2,axis=1)
    #############################################################################
    #                                                                           #
    #                             END OF YOUR CODE                              #
    #                                                                           #
