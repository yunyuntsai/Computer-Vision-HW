...
def makeTransformToReferenceFrame(i_To_iPlusOne_Transform, currentFrameIndex, refFrameIndex):
    ...
    print(i_To_iPlusOne_Transform[0])
    print(currentFrameIndex)
    print(refFrameIndex)

    if currentFrameIndex + 1 == refFrameIndex:
        T = i_To_iPlusOne_Transform[currentFrameIndex]
    elif currentFrameIndex != refFrameIndex:
        print(currentFrameIndex,refFrameIndex)

        if refFrameIndex > currentFrameIndex:
            candidat_index = np.arange(currentFrameIndex,refFrameIndex,1)
        else:
            candidat_index = np.arange(currentFrameIndex - 1,refFrameIndex - 1,-1)
        T = np.eye(i_To_iPlusOne_Transform[candidat_index[0]].shape[0])
        for ii in range(len(candidat_index)):
            if refFrameIndex > currentFrameIndex:
                T = np.matmul(i_To_iPlusOne_Transform[candidat_index[ii]],T)
            else:
                T = np.matmul(np.linalg.pinv(i_To_iPlusOne_Transform[candidat_index[ii]]),T)
    elif currentFrameIndex == refFrameIndex:
        T = np.eye(i_To_iPlusOne_Transform[0].shape[0])
