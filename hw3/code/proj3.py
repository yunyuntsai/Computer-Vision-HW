...
def main():
    ...
    for i in range(0,len(CATEGORIES)):
        catgo[CATEGORIES[i]] = 0.0
    for i in range(0,len(CATEGORIES)):
        total = 0.0
        for j in zip(test_labels,predicted_categories):
            if(j[0]==CATEGORIES[i]):
                total += 1.0
                if(j[0]==j[1]):
                    catgo[CATEGORIES[i]] += 1.0
        catgo[CATEGORIES[i]] = float(catgo[CATEGORIES[i]])/float(total)
    print(catgo)
    print("Accuracy = ", accuracy)
